...
/**
 * @author Rizwan - Sprint 35 - Regression Test AT-219413: To Verify claim's status, Days remaining in current phase and Last status
 *         update after Stage 2 settlement pack is received
 * 
 * @Pre-req MOJ Claim status should be in “Stage 2 settlement pack decision”
 * 
 * @ExpectedResults After Stage 2 settlement pack received, Claim's status should be "Stage 2 settlement pack decision", Days
 *                  remaining in current phase should be "2" and Last status update should be the current date
 */

public class TEST_AT219413_Stage2SettlementPackRecieved_StatusUpdated extends DriverManager
{
	@Parameters({ "strBrowser", "strEnvironmentName", "strEnvironmentBrand", "strRegionValue", "strCompany" })
	@Test
	public void testForAT219413_Stage2SettlementPackRecieved_StatusUpdated (@Optional("CHROME") String strBrowser,
			...
			@Optional("UK") String strRegionValue, @Optional("8880") String strCompany) throws Exception
	{
		...
		String strDaysRemainingInCurrentPhase = "2";
		String strLastPhaseUpdateValue;
		...
		// Validate Claim's status, Days remaining in current and Last status update
		...
		strLastPhaseUpdateValue = CalenderDate.returnCurrentDate (strRegionValue);
		mojactions.validateLastPhaseUpdateNew (strLastPhaseUpdateValue);
