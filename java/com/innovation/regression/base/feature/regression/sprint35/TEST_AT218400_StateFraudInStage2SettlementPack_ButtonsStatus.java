...
/**
 * @author Rizwan - Sprint 35 - Regression Test AT-218400: To Verify certain buttons on MOJ actions frame are not displayed after
 *         State Fraud in stage 2 settlement pack buttons
 * @Pre-req MOJ Claim status should be in “Stage 2 settlement pack decision”
 * 
 * @ExpectedResults After confirming stating of fraud, 'Request timeout extension', 'Send decision', 'State fraud' and 'Exit
 *                  process' buttons are not displayed
 * 
 * 
 */

public class TEST_AT218400_StateFraudInStage2SettlementPack_ButtonsStatus extends DriverManager
{
	@Parameters({ "strBrowser", "strEnvironmentName", "strEnvironmentBrand", "strRegionValue", "strCompany" })
	@Test
	public void testForAT218400_StateFraudInStage2SettlementPack_ButtonsStatus (@Optional("CHROME") String strBrowser,
			...
			@Optional("UK") String strRegionValue, @Optional("8880") String strCompany) throws Exception
	{
		...
		// Validate 'Request timeout extension', 'Send decision', 'State fraud' and 'Exit process' buttons are not displayed
		seleniumwaiter.waitForMe (GlobalWaitTime.getIntWaitTime ());
		mojactions.validateRequestTimeoutExtensionNotDisplayedStage2Settlement ();
		mojactions.validateSendDecisionNotDisplayedStage2Settlement ();
		mojactions.validateStateFraudNotDisplayed ();
		mojactions.validateExitProcessNotDisplayed ();
