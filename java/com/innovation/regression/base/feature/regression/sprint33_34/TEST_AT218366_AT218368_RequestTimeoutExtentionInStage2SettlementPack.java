...
/**
 * @author Rizwan - Sprint 33 - Regression Test AT-218366 & AT-218368: To Verify successful timeout extension request in stage 2
 *         settlement pack
 * 
 * @Pre-req MOJ Claim status should be in “Stage 2 settlement pack decision”
 * 
 * @ExpectedResults After requesting timeout extension in stage 2 settlement pack decision, claim's status should remain the same.
 *                  Days remaining in current phase should be increased as per the revised timeout date
 * 
 */

public class TEST_AT218366_AT218368_RequestTimeoutExtentionInStage2SettlementPack extends DriverManager
{
	@Parameters({ "strBrowser", "strEnvironmentName", "strEnvironmentBrand", "strRegionValue", "strCompany" })
	@Test
	public void testForAT218366_AT218368RequestTimeoutExtentionInStage2SettlementPack (@Optional("CHROME") String strBrowser,
			...
			@Optional("UK") String strRegionValue, @Optional("8880") String strCompany) throws Exception
	{
		...
		String strClaimStatusNew = "Stage 2 settlement pack decision";
		...
		// Validate Claim's status is not changed and it is 'Stage 2 settlement pack decision'
		seleniumwaiter.waitForMe (GlobalWaitTime.getIntWaitTime ());
		mojactions.validateClaimStatusNew (strClaimStatusNew);

		// Validate Days remaining in current phase after requesting Timeout extension
		// Days remaining in current phase should be the same as the number days extended from today
		String strDaysRemainingInCurrentPhase = "5";
