...
/**
 * @author Rizwan - Sprint 33 - Regression Test AT-217955: To Verify successful confirmation of stage 2 settlement pack
 * 
 * @Pre-req MOJ Claim status should be in “Stage 2 settlement pack decision”
 * 
 * @ExpectedResults After confirming Stage 2 settlement pack, Claim's status should be "Stage 2 settlement pack confirmation end"
 *                  and Days remaining in current phase should be "none", Net value of offer should be the same as the net damages
 *                  claimed, all loss requests in Stage 2 Loss Request table are marked as complete
 * 
 */

public class TEST_AT217955_ConfirmStage2SettlementPack extends DriverManager
{
	@Parameters({ "strBrowser", "strEnvironmentName", "strEnvironmentBrand", "strRegionValue", "strCompany" })
	@Test
	public void testForAT217955ConfirmStage2SettlementPack (@Optional("CHROME") String strBrowser, @Optional("BaseDevelopment") String strEnvironmentName,
			...
			throws Exception
	{
		...
		String strSettlementPackDecision = "Confirm";
		String strInterimPaymentAmounts = "0";
		String strClaimStatusNew = "Stage 2 settlement pack confirmation end";
		...
		// Confirm Stage 2 Settlement pack
		seleniumwaiter.waitForMe (GlobalWaitTime.getIntWaitTime ());
		...
		// Validate Claim's status and Days remaining in current phase after confirming Stage 2 Settlement pack
		...
		// Validate Total net value of offer is equal to Total net damages claimed
		String strTotalNetDamagesClaimed = mojactions.returnTotalNetDamagesClaimedTodateStage2Settlement ();
		mojactions.validateNetValueOfOfferTodateStage2Settlement (strTotalNetDamagesClaimed);

		// Validate all the loss requests in Stage 2 Loss Request table are complete
		mojactions.validateStage2LossRequestsCompleted ();
