...
/**
 * @author Rizwan - Sprint 33 - Regression Test AT-218402: To Verify successful repudiation of stage 2 settlement pack
 * 
 * @Pre-req MOJ Claim status should be in “Stage 2 settlement pack decision”
 * 
 * @ExpectedResults After repudiating Stage 2 settlement pack, Claim's status should be "Stage 2 settlement pack repudiation" and
 *                  Days remaining in current phase should be "none", Last phase update should be the current date
 * 
 */

public class TEST_AT218402_RepudiateStage2SettlementPack extends DriverManager
{
	@Parameters({ "strBrowser", "strEnvironmentName", "strEnvironmentBrand", "strRegionValue", "strCompany" })
	@Test
	public void testForAT218402RepudiateStage2SettlementPack (@Optional("CHROME") String strBrowser, @Optional("BaseDevelopment") String strEnvironmentName,
			...
			throws Exception
	{
		...
		String strClaimStatusNew = "Stage 2 settlement pack repudiation";
		...
		// Validate Claim's status after repudiating Stage 2 Settlement pack
		seleniumwaiter.waitForMe (GlobalWaitTime.getIntWaitTime ());
		mojactions.validateClaimStatusNew (strClaimStatusNew);

		// Validate Days remaining in current phase after repudiating Stage 2 Settlement pack
		mojactions.validateDaysRemainingInCurrentPhaseNew (strDaysRemainingInCurrentPhase);

		// Validate Last phase update after repudiating Stage 2 Settlement pack
		strLastPhaseUpdate = CalenderDate.returnCurrentDate (strRegionValue);
		mojactions.validateLastPhaseUpdateNew (strLastPhaseUpdate);
