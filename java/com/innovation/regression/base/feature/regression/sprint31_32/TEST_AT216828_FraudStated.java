...
/**
 * @author haiderm - Sprint 31 - Regression Test AT-216828: To verify that when user clicks Confirm and message is successful,
 *         status should be updated LOB.
 * @Pre-req MOJ Claim status should be in “Interim payment decision” status.
 * 
 * @ExpectedResults Confirm should send a message to the Claimant Representative to state that fraud is suspected System should
 *                  update the status of the record to “Fraud Stated” (MOJ status = “FraudStated”). Days remaining should set to
 *                  null to make sure that CNF record has exited the process.
 *
 */
public class TEST_AT216828_FraudStated extends DriverManager
{
			...
			throws Exception
	{
		...
		List<String> lstSelectedTimeoutDate;
		String strSelectedTimeoutDate;
		String strPhaseCacheCode;
		String strFraudStated = "13"; // PhaseCacheCode for Fraud stated is 13
		...
		if (strMOJClaimStatus.equalsIgnoreCase ("Interim Payment decision"))
		{
			...
			// Step 5: Click on <State Fraud> button
			mojactions.clickStateFraud ();

			// Step 6: Enter fraud comments in text field and click <Save>
			...
			seleniumwaiter.waitForMe (GlobalWaitTime.getIntWaitTime ());
			strSQLQuery = MOJSQL.returnSelectedTimeoutDate (strMOJClaimNumber);
			lstSelectedTimeoutDate = DatabaseData.returnDatabaseValue (strSQLQuery, strEnvironmentName);
			strSelectedTimeoutDate = lstSelectedTimeoutDate.get (0);

			if (strPhaseCacheCode.equalsIgnoreCase (strFraudStated) && strMOJCurrentStatus.equalsIgnoreCase (strMOJClaimStatus))
			{
			...
			if (strSelectedTimeoutDate.equalsIgnoreCase ("99999999"))
			{
				System.out.println ("Days Remaining in Current Phase are :" + "Null");
			}
			else
			{
				throw new TestCaseValidationFailed ();
