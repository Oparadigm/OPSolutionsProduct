...
/**
 * @author Rizwan. Regression test script covers acceptance of partial interim settlement pack. After accepting partial interim
 *         settlement pack, it validates claim's current status and days remaining in current phase. It then validates tasks
 *         generated and completed.
 *
 */

public class TEST_AT216836_AcceptPartialInterimSettlement_StatusUpdated extends DriverManager
{

	@Parameters({ "strBrowser", "strEnvironmentName", "strEnvironmentBrand", "strRegionValue", "strCompany" })
	@Test
	public void acceptPartialInterimSettlement_StatusUpdated (@Optional("CHROME") String strBrowser, @Optional("BaseDevelopment") String strEnvironmentName,
			...
			throws Exception
	{
		...
		seleniumwaiter.waitForMe (GlobalWaitTime.getIntWaitTime ());
		strClaimStatusNew = "Acceptance of partial interim payment";
		mojactions.validateClaimStatusNew (strClaimStatusNew);
		...
		mojsummary.validateAuditGeneration (strNewTaskGenerated);
