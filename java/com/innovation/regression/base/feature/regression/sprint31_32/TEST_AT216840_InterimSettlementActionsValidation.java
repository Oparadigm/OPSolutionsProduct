...
/**
 * @author haiderm - Sprint 31 - Regression Test AT-216840: To verify that the Interim settlement user reviewing is the additional
 *         request, all actions should be available to perform
 * @Pre-req MOJ Claim status should be in “Interim payment decision” status and there must be one or more settlement requests
 *          already be received
 * 
 * @ExpectedResults Request should be displayed as a new row in the table on the Interim Settlement tab and all functionality
 *                  inlcuding (reject, accept partially, accept fully, exit process and state fraud) should be available for this
 *                  request when the new row is expanded.
 *
 */
public class TEST_AT216840_InterimSettlementActionsValidation extends DriverManager
{
	@Parameters({ "strBrowser", "strEnvironmentName", "strEnvironmentBrand", "strRegionValue", "strCompany" })
	@Test
	public void testForAT216840 (@Optional("CHROME") String strBrowser, @Optional("BaseDevelopment") String strEnvironmentName,
			...
			throws Exception
	{
		...
		if (strPhaseCacheName.equalsIgnoreCase ("Interim Payment decision"))
		{
			...
			if (mojactions.validateRejectRequest () == true && mojactions.validateSendDecision () == true

				&& mojactions.validateExitProcess () == true && mojactions.validateStateFraud ())
			{
				System.out.println ("Reject request functionality is available.");
				System.out.println ("Accept fully / partially request functionality is available.");
				System.out.println ("State fraud functionality is available.");
				System.out.println ("Exit process functionality is available.");
